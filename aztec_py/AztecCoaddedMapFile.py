...
class AztecCoaddedMapFile():
    ...
    def getAll(self):
        ...
        self.getHistBins()
        self.getHistVals()
        self.getHistBinsFilteredSignal()
        self.getHistValsFilteredSignal()
    ...
    def getFilteredSignalMap(self):
        self.filteredSignal = self.nc.variables['filteredSignal'][:]
    ...
    def getHistBinsFilteredSignal(self):
        self.histBinsFilteredSignal = self.nc.variables['histBins_filteredSignal'][:]

    def getHistValsFilteredSignal(self):
        self.histValsFilteredSignal = self.nc.variables['histVals_filteredSignal'][:]
    ...
    def plotHistogram(self, normalize=0, log=0, legend=0, filtered=0):
        if(not filtered):
            ...
            try: 
                self.histBinsFilteredSignal
            except:
                self.getHistBinsFilteredSignal()
                self.getHistValsFilteredSignal()
            plotbins = self.histBinsFilteredSignal
            plotvals = self.histValsFilteredSignal
    ...
    def fitHistogram(self, filtered=0):
        if(not filtered):
            ...
            try: 
                ...
                self.getHistBinsFilteredSignal()
                self.getHistValsFilteredSignal()
            fitbins = np.array(self.histBinsFilteredSignal)
            fitvals = np.array(self.histValsFilteredSignal)
