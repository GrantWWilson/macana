...
class Map:
    ...
    def generateRGrid(self):
        """Generate a matrix object with the distance relative to the map center"""
        xx,yy = npy.meshgrid(self.RaCoords-self.RaCenter, self.DecCoords-self.DecCenter)
        xx = xx.transpose()
        yy = yy.transpose()
...
#             
    ...
    def loadFromNcFile(self, ncFile):
            ...
            try:
                self.source = getattr(ncFile,"source")
            ...
            self.RaCenter = 0.0
            self.DecCenter = 0.0
            self.AbsRaCoords = npy.array(ncFile.variables['xCoordsAbs'][:])*units.deg
            self.AbsDecCoords = npy.array(ncFile.variables['yCoordsAbs'][:])*units.deg
            try:
                self.kernel = npy.array(ncFile.variables['kernel'][:])
            except KeyError:
                self.kernel = None
            try:
                self.SourceRa = getattr(ncFile, "MasterGrid[0]")*180.0/npy.pi
                self.SourceDec = getattr(ncFile, "MasterGrid[1]")*180.0/npy.pi
            except AttributeError:
                self.SourceRa = getattr(ncFile, "RaCenter")*180.0/npy.pi
                self.SourceDec = getattr(ncFile, "DecCenter")*180.0/npy.pi
            try:
            ...
            if self.nSources > 0:
                for i in range (self.nSources):
                    ...
                    rapos = ncFile.variables[svar].raCentroid
                    decpos = ncFile.variables[svar].decCentroid
    ...
    def loadFromIDLSav(self,savHandler):
            self.filtered = True
            self.signal = npy.array(savHandler.coadded_signal_map, copy = True)*units.Jy
            self.fSignal = self.signal
            self.weight = npy.array(savHandler.coadded_weight_map, copy = True)* (1.0/units.Jy)**2.0
            ...
            self.AbsRaCoords = npy.array(savHandler.abs_ra_map,copy = True)
            self.AbsDecCoords = npy.array(savHandler.abs_dec_map,copy = True)
            self.kernel = savHandler.kernel_mean
    ...
    def wcut (self,wcut):
        """This removes all pixel from signal,weight and SN maps that are below the wcut percentile value on the weight map
            Input:
                    wcut    Threshold for cutting, all the pixels below this percentile value from 
                            the weight map will be se to 0.0
        """
        if self.filtered:
            wmap = self.fWeight
        else:
        ...
        covcut = npy.percentile(wmap, wcut)
        w = npy.where(wmap.value < covcut)
        if len (w) > 0:
            self.signal[w]=0.0
            self.weight[w]=0.0
    ...
    def writeToNc(self,ncFileName):
        ncFile = NetCDFFile(ncFileName, 'w', format='NETCDF3_64BIT')
        ...
        setattr(ncFile, "source", self.source)
        setattr(ncFile, "MasterGrid[0]", self.RaCenter/180.0*(npy.pi))
        setattr(ncFile, "MasterGrid[1]", self.DecCenter/180.0*(npy.pi))
        ...
        ncFile.variables['xCoordsAbs'][:] = self.AbsRaCoords.value
        ncFile.variables['yCoordsAbs'][:] = self.AbsDecCoords.value
        if self.filtered:
